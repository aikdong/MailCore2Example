...
- (void)setupImageAndDownloadIfNecessary:(JTSImageInfo *)imageInfo {
    if (imageInfo.image) {
        ...
        [SDWebImageManager.sharedManager downloadImageWithURL:imageInfo.imageURL options:SDWebImageHighPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            float progress = 0;
            if (expectedSize > 0 && _flags.imageIsBeingReadFromDisk == NO) {
