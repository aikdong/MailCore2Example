...
- (void)reloadDataAnimated:(BOOL)animated {
    ...
    if (!animated) {
		...
		// This issue： Terminating app due to uncaught exception 'NSInternalInconsistencyException', reason: 'attempt to delete and reload the same index path (<NSIndexPath: 0xc000000000018016> {length = 2, path = 0 - 3})'
		NSPredicate *thePredicate1 = [NSPredicate predicateWithFormat:@"NOT (SELF in %@)", self.originalTable.insertIndexPaths];
		[self.originalTable.updateIndexPaths filterUsingPredicate:thePredicate1];
		NSPredicate *thePredicate2 = [NSPredicate predicateWithFormat:@"NOT (SELF in %@)", self.originalTable.deleteIndexPaths];
		[self.originalTable.updateIndexPaths filterUsingPredicate:thePredicate2];
