...
#define MAX_MEMORY_SIZE 22 // The maximum file size that will be cached (2^22 = 4MB)
///
/// Uncomment CACHE_DEBUG_MODE to enable logging
///
//#define LOG_DEBUG_MODE
#ifdef LOG_DEBUG_MODE
#define TBDebugLog( s, ... ) NSLog(@"<%@:(%d)> %@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define TBDebugLog( s, ... )
#endif
