...
+(void)activate {
    // set caching paths
	_cacheDirectory = [[[ApplicationDirector sharedInstance] cacheDirectory] stringByAppendingPathComponent:CACHE_FOLDER];
    ...
    // activate cache
	EVURLCache *urlCache = [[EVURLCache alloc] initWithMemoryCapacity:(1<<MAX_MEMORY_SIZE) diskCapacity:(1<<MAX_FILE_SIZE) diskPath:_cacheDirectory] ;
...
-(NSCachedURLResponse *)cachedResponseForRequest:(NSURLRequest *)request {
    TBDebugLog(@"CACHE REQUEST %@", request);
    
    // is caching allowed
    if ((request.cachePolicy == NSURLCacheStorageNotAllowed || [request.URL.absoluteString hasPrefix:@"file://"] || [request.URL.absoluteString hasPrefix:@"data:"]) && AFNetworkReachabilityManager.sharedManager.reachable) {
		TBDebugLog(@"CACHE not allowed for %@", request.URL);
		return nil;
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:storagePath]) {
		TBDebugLog(@"CACHE found for %@", storagePath);
        ...
        if (AFNetworkReachabilityManager.sharedManager.reachable) {
            ...
            if (modificationTimeSinceNow > threshold) {
                TBDebugLog(@"CACHE item older than %@ maxAgeHours", maxAge);
                return nil;
            }
            TBDebugLog(@"CACHE max age = %@, file date = %@", maxAge, modDate);
        }
    ...
    TBDebugLog(@"CACHE not found %@", storagePath);
    return nil;
...
- (void)storeCachedResponse:(NSCachedURLResponse *)cachedResponse forRequest:(NSURLRequest *)request {
    ...
    if (request.cachePolicy == NSURLCacheStorageNotAllowed) {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:storagePath]) {
            TBDebugLog(@"CACHE not storing file, it's not allowed by the cachePolicy : %@", request.URL);
            return;
        }
        TBDebugLog(@"CACHE file in PreCache folder, overriding cachePolicy : %@", request.URL);
    }
	...
	if (![[NSFileManager defaultManager] createDirectoryAtPath:storageDirectory withIntermediateDirectories:YES attributes:nil error:&error]) {
		TBDebugLog(@"Error creating cache directory: %@", error);
	}
    
    // save file
    TBDebugLog(@"Writing data to %@", storagePath);
	if (![[cachedResponse data] writeToFile:storagePath atomically:YES]) {
		TBDebugLog(@"Could not write file to cache");
	} else {
        ...
        if (![EVURLCache addSkipBackupAttributeToItemAtURL:cacheURL]){
            TBDebugLog(@"Could not set the do not backup attribute");
        }
    }
...
+ (BOOL)addSkipBackupAttributeToItemAtURL:(NSURL *)URL {
    ...
    NSUInteger result = getxattr(filePath, attrName, NULL, sizeof(u_int8_t), 0, 0);
    if (result != -1) {
        // The attribute exists, we need to remove it
        int removeResult = removexattr(filePath, attrName, 0);
        if (removeResult == 0) {
            TBDebugLog(@"Removed extended attribute on file %@", URL);
        }
    }
